...
static uint16_t ui_best_emg_peak = 0U;
static uint16_t ui_best_rehab_score = 0U;

static lv_timer_t *rehab_timer = NULL;
static int32_t rehab_left_ms = 10000;   // 10秒
static uint8_t rehab_running_State = 0;
static uint32_t rehab_start_tick = 0;
static uint32_t rehab_duration_ms = 10000;
...
typedef struct
{
    ...
    lv_obj_t * emg_chart;
    lv_chart_series_t * emg_level_series;
...
static int16_t ui_finger_angle_deg(uint8_t finger_id)
{
    int16_t angle_deg;
    ...
    if(angle_deg < 49) return 49;
    if(angle_deg > 190) return 190;
    return angle_deg;
}
...
static uint16_t ui_finger_percent(uint8_t finger_id)
{
    ...
    angle_deg = ui_finger_angle_deg(finger_id);
...
static void ui_build_home(void)
{
    ...
    ui_create_mode_card(s_ui.home_scr, 410, 242, 370, 120,
                        "System Status",
                        "View stored parameters and records.",
...
static void ui_build_free_page(void)
{
    ...
    lv_label_set_text(title_left, "Root Joint Angle");
    ...
    for(i = 0; i < 5; i++)
    {
        ...
        lv_label_set_text(s_ui.free_values[i], "49 deg");
    ...
    lv_label_set_text(s_ui.free_status_label, "Status: waiting | Calib: 0/0");
    ...
    lv_label_set_text(s_ui.free_calib_label, "");
    ui_set_label_font(s_ui.free_calib_label, &lv_font_montserrat_14, color_muted());
    lv_obj_set_pos(s_ui.free_calib_label, 0, 334);
    lv_obj_add_flag(s_ui.free_calib_label, LV_OBJ_FLAG_HIDDEN);
}


static const char * gesture_names[3] =
{
    "Open",
    "Index",
    "Clench"
...
static const uint8_t rehab_target_table[3][5] =
{
    {0U, 0U, 0U, 0U, 0U},
    {0U, 100U, 0U, 0U, 0U},
...
static uint16_t rehab_calc_score(void)
{
    ...
    if((rehab_last_gesture < 0) || (rehab_last_gesture >= 3))
    {
        return 0U;
    }
...
static void rehab_update_score_label(const char *prefix, uint16_t score)
{
    ...
    lv_obj_center(s_ui.rehab_score_label);
}
...
{
    rehab_current_score = rehab_calc_score();
...
static void rehab_show_next_gesture()
{
    ...
    do
    {
        ...
        lv_obj_center(s_ui.rehab_target_text);
}
...
static void rehab_timer_cb(lv_timer_t * timer)
{
    ...
    else
    {
        ...
        if(rehab_score_avg > ui_best_rehab_score)
        {
            ui_best_rehab_score = rehab_score_avg;
        }
...
static void ui_build_rehab_page(void)
{
   ...
   lv_obj_add_event_cb(s_ui.rehab_scr, ui_page_gesture_back_cb, LV_EVENT_GESTURE, NULL);
    ...
    ui_set_label_font(s_ui.rehab_target_text,&lv_font_montserrat_32,lv_color_hex(0xFFFFFF));
    ...
    lv_label_set_text(s_ui.rehab_time_label,"00:10");
    ...
    ui_set_label_font(s_ui.rehab_score_label, &lv_font_montserrat_22, color_green());
    lv_obj_center(s_ui.rehab_score_label);
...
static void ui_build_emg_page(void)
{
   lv_obj_t *left;
    ...
    lv_obj_set_pos(s_ui.emg_timer_label, 0, -100);

    desc = ui_create_panel(right, 0, 148, 500, 180);

    lbl = lv_label_create(desc);
    lv_label_set_text(lbl, "Force Trend");
    ui_set_label_font(lbl, &lv_font_montserrat_20, color_text());
    lv_obj_set_pos(lbl, 0, 0);

    s_ui.emg_chart = lv_chart_create(desc);
    lv_obj_set_size(s_ui.emg_chart, 468, 112);
    lv_obj_set_pos(s_ui.emg_chart, 0, 38);
    lv_chart_set_type(s_ui.emg_chart, LV_CHART_TYPE_LINE);
    lv_chart_set_range(s_ui.emg_chart, LV_CHART_AXIS_PRIMARY_Y, 0, 100);
    lv_chart_set_point_count(s_ui.emg_chart, 48);
    lv_chart_set_update_mode(s_ui.emg_chart, LV_CHART_UPDATE_MODE_SHIFT);
    lv_chart_set_div_line_count(s_ui.emg_chart, 5, 5);
    lv_obj_set_style_bg_color(s_ui.emg_chart, lv_color_hex(0x0B0F16), 0);
    lv_obj_set_style_bg_opa(s_ui.emg_chart, LV_OPA_100, 0);
    lv_obj_set_style_border_color(s_ui.emg_chart, color_line(), 0);
    lv_obj_set_style_radius(s_ui.emg_chart, 6, 0);
    lv_obj_set_style_line_width(s_ui.emg_chart, 2, LV_PART_ITEMS);
    lv_obj_set_style_size(s_ui.emg_chart, 0, LV_PART_INDICATOR);
    s_ui.emg_level_series = lv_chart_add_series(s_ui.emg_chart,
                                                color_accent(),
                                                LV_CHART_AXIS_PRIMARY_Y);
    for(i = 0U; i < 48U; i++)
    {
        lv_chart_set_next_value(s_ui.emg_chart, s_ui.emg_level_series, 0);
    }
}
...
static void ui_build_calib_page(void)
{
    ...
    s_ui.calib_record_label = lv_label_create(left);
    lv_obj_set_width(s_ui.calib_record_label, 220);
    lv_label_set_long_mode(s_ui.calib_record_label, LV_LABEL_LONG_WRAP);
    lv_label_set_text(s_ui.calib_record_label,
                      "Best EMG peak: --%\n"
                      "Best rehab score: --\n"
                      "Setup: open-close-open");
    ui_set_label_font(s_ui.calib_record_label, &lv_font_montserrat_16, color_muted());
    lv_obj_set_pos(s_ui.calib_record_label, 0, 254);
...
static void ui_refresh_runtime(lv_timer_t *timer)
{
    ...
    if(s_ui.current_page == UI_PAGE_FREE)
    {
        for(i = 0; i < 5; i++)
        {
            int16_t angle_deg = ui_finger_angle_deg(i);
            ...
            {
                ui_label_set_text_fmt_u16(s_ui.free_values[i], "%u deg", (unsigned int)angle_deg);
            }
        }
        ...
        if(s_ui.free_status_label != NULL)
        {
            ...
            if(enabled == 0U)
            {
                status_text = "Status: no enabled root fingers";
            }
            else if(calibrated == enabled)
            {
                status_text = "Status: mapping task ready";
            }
            else
            {
                status_text = "Status: waiting for calibration";
            }
            ...
            ui_label_set_text_if_changed(s_ui.free_status_label, buf);
        }
     ...
     if(s_ui.current_page == UI_PAGE_EMG)
    {
        ...
        if(emg.peak > ui_best_emg_peak)
        {
            ui_best_emg_peak = emg.peak;
        }
        ...
        if((s_ui.emg_chart != NULL) && (s_ui.emg_level_series != NULL))
        {
            lv_chart_set_next_value(s_ui.emg_chart,
                                    s_ui.emg_level_series,
                                    (lv_coord_t)emg.level);
        }
    ...
    {
        LV_UNUSED(enabled);
        LV_UNUSED(calibrated);
        ui_label_set_text_if_changed(s_ui.calib_ready_label, "READY");

        if(s_ui.calib_channels_label != NULL)
        {
            ui_label_set_text_if_changed(s_ui.calib_channels_label,
                                         "Flex profiles active\n"
                                         "EMG records ready\n"
                                         "Rehab scoring ready");
        }

        if(s_ui.calib_record_label != NULL)
        {
            snprintf(buf,
                     sizeof(buf),
                     "Best EMG peak: %u%%\nBest rehab score: %u\nSetup: open-close-open",
                     (unsigned int)ui_best_emg_peak,
                     (unsigned int)ui_best_rehab_score);
            ui_label_set_text_if_changed(s_ui.calib_record_label, buf);
        }
    }
}
